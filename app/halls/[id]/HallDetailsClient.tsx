...
export default function FeedPage() {
  ...
  useEffect(() => {
    ...
    // 1. Check for approved demo films in localStorage
    if (typeof window !== 'undefined') {
      const demoFilmsStr = localStorage.getItem('nexa-demo-films');
      const demoStudiosStr = localStorage.getItem('nexa-demo-studios');
      
      if (demoFilmsStr) {
        try {
          const demoFilms = JSON.parse(demoFilmsStr);
          const demoStudios = JSON.parse(demoStudiosStr || '[]');
          
          const approvedDemoFilms = demoFilms.filter((f: any) => f.status === 'approved');
          
          if (approvedDemoFilms.length > 0) {
            const transformed = approvedDemoFilms.map((f: any) => {
              const studio = demoStudios.find((s: any) => s.id === f.studio_id) || { name: 'Demo Studio' };
              return {
                id: f.id,
                title: f.title,
                thumbnail: f.poster_url || '/scifi-hall.png',
                videoUrl: f.trailer_url,
                studio: studio.name,
                studioId: f.studio_id,
                studioAvatar: studio.name.charAt(0),
                views: '0',
                timeAgo: 'Just now',
                duration: 'Feature',
                likes: '0',
          ...
          console.error("Error parsing demo films:", e);
        }
      }
    }

    // 2. Check if the user posted a new custom feed trailer from the dashboard (Shorts/Posts)
    if (typeof window !== 'undefined') {
